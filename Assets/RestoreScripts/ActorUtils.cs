...
namespace MoleMole
{
	...
	public static class ActorUtils // TypeDefIndex: 21050
	{
		...
		// Methods
		[BlackList]
		public static void AddCacheCmd(string alias, BaseCommand cmd) 
		{
			MoleMole.ActorUtils.infoMgr.GetEntityMetaByAlias(alias).AddCacheCmd(cmd);
		}
		
		[BlackList]
		public static void ClearCacheCmd(string alias, ActorCommandType cmdType) 
		{
			MoleMole.ActorUtils.infoMgr.GetEntityMetaByAlias(alias).ClearCacheCmd(cmdType);
		}
		
		public static void ClearCacheCmdExternal(string alias, int cmdType) 
		{
			MoleMole.ActorUtils.infoMgr.GetEntityMetaByAlias(alias).ClearCacheCmd(cmdType);
		}
		
		public static void ClearCacheCmdAll(string alias) 
		{
			MoleMole.ActorUtils.infoMgr.GetEntityMetaByAlias(alias).ClearAllCacheCmd();
		}
		
		public static void DoFreeStyle(string alias, int freeStyleType, bool forceInterrupt, float? freeStyleParam = default, bool resetRoute = true /* Metadata: 0x00AFED5C */) 
		{
			MoleMole.ActorUtils.AddCacheCmd(alias, FreeStyleCmd.Get(freeStyleType, forceInterrupt, freeStyleParam, resetRoute));
		}
		
		public static void DoLookAt(string alias, Vector3 pos) 
		{
			MoleMole.ActorUtils.AddCacheCmd(alias, LookAtCmd.Get(pos));
		}
		
		public static void DoSitOnChair(string alias, int freeStyleType) 
		{
			SitOnChairCmd Cmd = SitOnChairCmd.Get();
			Cmd.freeStyleType = freeStyleType;
			MoleMole.ActorUtils.AddCacheCmd(alias, Cmd);
		}
		
		public static void DoSetWalkSpeedRatio(string alias, float speedRatio) 
		{
			SetWalkSpeedCmd Cmd = SetWalkSpeedCmd.Get();
			Cmd.walkSpeedRatio = speedRatio;
			MoleMole.ActorUtils.AddCacheCmd(alias, Cmd);
		}
		
		public static void DoResetWalkSpeedRatio(string alias) 
		{
			MoleMole.ActorUtils.ClearCacheCmd(alias, ActorCommandType.WALK_SPEED);
		}
		
		public static void DoStartCall(string alias, Action<ILuaActor> call) 
		{
			MoleMole.ActorUtils.AddCacheCmd(alias, StartCallCmd.Get(call));
		}
		
		public static void DoAddGeneralMark(string alias, string iconName) 
		{
			MoleMole.ActorUtils.AddCacheCmd(alias, AddGeneralMarkCmd.Get(iconName));
		}
		
		public static void DoClearGeneralMark(string alias) 
		{
			MoleMole.ActorUtils.ClearCacheCmd(alias, ActorCommandType.ADD_GENERAL_MARK);
		}
		
		public static void DestroyLocalGadget(string alias) 
		{
			MoleMole.ActorUtils.infoMgr.ReleaseLocalEntityInfoData(alias);
		}
		
		public static void DoHide(string alias, bool hide) 
		{
			if (hide)
			{
				MoleMole.ActorUtils.AddCacheCmd(alias, HideCmd.Get(hide));
			}
			else
			{
				MoleMole.ActorUtils.ClearCacheCmd(alias, ActorCommandType.HIDE);
			}
		}
		
		public static void DisableInteeHeadCtrl(string alias, bool isDisable) 
		{
			MoleMole.ActorUtils.AddCacheCmd(alias, DisableInteeHeadCtrlCmd.Get(isDisable));
		}
		...
		public static void RefreshGlobalHandler() 
		{
			MoleMole.ActorUtils.RefreshAvatarSitStandHandler();
			MoleMole.ActorUtils.RefreshElemViewHandler();
		}
		
		public static void ShowMessageByTextMap(string mapID) 
		{
			UIUtil.TryShowMessage(TextMap.Get(mapID, false));
		}
		
		public static void ShowTextMapWithParam(string mapID, string[] param) 
		{
			string Text;
			if (param != "")
			{
				Text = string.Format(param, TextMap.Get(mapID, false));
			}
			else
			{
				Text = TextMap.Get(mapID, false);
			}
			UIUtil.TryShowMessage(Text);
		}
		
		public static void ShowMessage(string content) 
		{
			UIUtil.TryShowMessage(content);
		}
		
		public static void UnrequireLua(string luaPath) 
		{
			MoleMole.CommonMiscs.UnrequireLua(MoleMole.Miscs.ConcatString(prePath, luaPath, postPath));
		}
