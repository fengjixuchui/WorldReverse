...
namespace miHoYoThread
{
    public class SchedulerMgr : ISchedulerMgr // TypeDefIndex: 9050
    {
        ...
        public IGraph GetGraph(int schedulerType) => _schedulers[schedulerType].GetGraph(); // 0x0000000187637510-0x0000000187637600
        ...
        public void UnSpawnScheduleTask(ISchedulerTask task)
        {
            int index = task.CompareTo(task);
        ...
        public void ScheduleResume(int schedulerType)
        {
            _schedulers[schedulerType].Resume();
        } // 0x0000000187638050-0x0000000187638140
        ...
        public void ScheduleEntry(int schedulerType, bool lockScene)
        {
            if (_isStarted)
            {
                string name = _consts.SchedulerTypeEntryName(schedulerType);
                UnityEngine.Profiling.Profiler.BeginSample(name);
                CheckRunner();
                _schedulers[schedulerType].ScheduleEntry(lockScene);
                UnityEngine.Profiling.Profiler.EndSample();
            }
        } // 0x0000000187637B30-0x0000000187637C90
          // [XID] // 0x00000001896A0F30-0x00000001896A0F50
        public void ScheduleMainThreadEntry(int schedulerType)
        {
            if (_isStarted)
            {
        ...
        } // 0x0000000187637E30-0x0000000187637F60
          // [XID] // 0x00000001896A8390-0x00000001896A83B0
        public void ScheduleWaitAllFinished(int schedulerType)
        {
            if (_isStarted)
            {
