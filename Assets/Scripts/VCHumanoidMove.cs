    ...
    public sealed class VCHumanoidMove : VCBaseMove // TypeDefIndex: 11884
    {
        ...
        { /* [XID] */ /* 0x0000000189A6D0D0-0x0000000189A6D0F0 */
            get
            {
                ...
                {
                    _scenePropPlugin = Singleton<EntityManager>.Instance.levelEntity.GetLCLevelComponent().GetPlugin();
                }
        ...
        public Vector3 GetCurrentWallNormal() => _humanoidMoveFSM.GetCurrentWallNormal().XZ(); // 0x0000000183BF7780-0x0000000183BF7880
        ...
        private void InitHumanoid()
        {
            ...
            var footPos = Vector3.up * (avatarShoesHeight + 0.5f) + _transform.position.XZ();
            _entity.SetDynamicPointPos("Foot", footPos);

            footPos.Set(0, footPos.y, 0);
            var kneePos = footPos * ((avatarModelHeight + 0.3f) + avatarShoesHeight) + _transform.position.XZ();
            _entity.SetDynamicPointPos("Knee", kneePos);

            kneePos.Set(0, kneePos.y, 0);
            var waistPos = kneePos * ((avatarModelHeight + 0.6f) + avatarShoesHeight) + _transform.position.XZ();
            _entity.SetDynamicPointPos("Waist", waistPos);

            waistPos.Set(0, kneePos.y, 0);
            var climbHeadPos = kneePos * ((avatarHeadExtraHeight + avatarModelClimbHeight) + avatarShoesHeight) + _transform.position.XZ();
        ...
        protected override void InitMoveCollider()
        {
            ...
            if (_entity.isAuthority)
            {
                ...
                MoveCollider1[0].center = (Vector3.up * (((avatarModelHeight * 8f) + avatarShoesHeight) + (avatarHeadExtraHeight * 0.5f))).XZ();

                MoveCollider1[1].radius = avatarRadius;
                MoveCollider1[1].height = (avatarRadius * 0.2f) + (avatarModelHeight * 0.3f);
                MoveCollider1[1].center = (Vector3.up * (((avatarModelHeight * 0.4505f) + avatarShoesHeight) + avatarRadius)).XZ();
                colRadius = 1f;
            }
            else
            {
                ...
                MoveCollider1[0].center = (Vector3.up * (((avatarHeadExtraHeight + avatarShoesHeight) * 0.5f) + (avatarModelHeight * 0.875f))).XZ();
            ...
            PushCollider[0].center = (Vector3.up * (avatarHeight * 0.65f)).XZ();
            ...
            PushColliderExtra[0].center = (Vector3.up * (avatarHeight * 0.65f)).XZ();
            ...
            airDummyMoveCollider.center = (Vector3.up * (avatarHeight * 0.5f)).XZ();
            ...
            airDummyPushCollider.center = (Vector3.up * (avatarHeight * 0.65f)).XZ();
            ...
            airDummyPushColliderExtra.center = (Vector3.up * (avatarHeight * 0.65f)).XZ();
        ...
        public override void DoBlink(bool preferInput = false /* Metadata: 0x00AEADE3 */, float dist = 8f /* Metadata: 0x00AEADE4 */)
        {
            if (_entity != null && _entity.isAlive)
            {
                ...
                {
                    Debug.LogWarning("AvatarDoBlink dist = 0");
                }
                else if (preferInput && dist < 0.0)
                {
                    Debug.LogWarning("AvatarDoBlink preferInput while dist < 0");
                }
                else
                {
                    _humanoidMoveData.tryDoBlink = true;
        ...
        public override bool isInMovePureStandbyAndStayInPlace()
        {
            ...
            Type myType = typeof(AnimatorStateInfo);
            FieldInfo animatorStateInfo = typeof(AnimatorStateInfo).GetField("m_Tag");
        ...
        public override void SetNonAnimatorMode(bool value, bool animatorAlways)
        {
            if (_moveData != null && _moveData.isInCutSceneMode != value)
            {
                ...
                {
                    _moveData.targetSteerDir = _entity.rootGameObject.transform.forward.XZ();
                    if (animator)
                    {
        ...
        public override bool TryInitialSyncMove(ref MoveSyncTask syncTask, bool setPosition) => _humanoidMoveFSM.InitFsmStateWithSyncTask(syncTask, setPosition); // 0x0000000183BF98C0-0x0000000183BF99B0
                                                                                                                                                                  // [XID] // 0x000000018981AE50-0x000000018981AE70
        public override bool CanInitFsmStateWithSyncTask(ref MoveSyncTask syncTask) => _humanoidMoveFSM.CanInitFsmStateWithSyncTask(syncTask); // 0x0000000183BF9240-0x0000000183BF9310
        ...
        public override void DoFreeStyle(MotionFlag freeStyleType, bool forceInterrupt, float? freeStyleParam = default)
        {
            ...
            if (vcPerform != null)
            {
                ...
                else if (!forceInterrupt && DoingFreeStyle())
                {
                    ...
                    _entity.SetAnimatorInt(AnimatorParamsData.MOTION_FLAG, _moveData.curMoveState);
                }
                return;
            }
            ...
            _moveData.curMoveState = VCMoveData.MotionFlag.FreeStyle;
            _entity.SetAnimatorInt(AnimatorParamsData.MOTION_FLAG, _moveData.curMoveState);
            _entity.SetAnimatorInt(AnimatorParamsData.PERFORM_DOING, VCMoveData.MotionFlag.Walk);
            _entity.ResetAnimatorTrigger(AnimatorParamsData.PERFORM_STATE_TRIGGER);
            vcPerform.DoPerform(freeStyleType);
        ...
        {
            _humanoidMoveData.sitOnChairPosition = position.XZ();
            _humanoidMoveData.sitOnChairRotationEular = rotationEular.XZ();
        ...
        public override void ResetFreeStyle()
        {
            ...
            if (_moveData.curMoveState == VCMoveData.MotionFlag.FreeStyle)
            {
                ...
                _entity.SetAnimatorInt(AnimatorParamsData.MOTION_FLAG, _moveData.curMoveState);
                _entity.SetAnimatorInt(AnimatorParamsData.PERFORM_ID, (VCMoveData.MotionFlag)(-1));
                _entity.SetAnimatorInt(AnimatorParamsData.PERFORM_DOING, VCMoveData.MotionFlag.Idle);
            }
            else
            {
                ResetFreeStyleOnly();
        ...
        public static Vector3 GetSafeAppearPos(Vector3 checkPosition, BaseEntity checkAvatarEntity)
        {
            ...
            for (int i = 0; i < _avatars.Count; i++)
            {
                ...
                if (avatar.IsActive())
                {
                    if (avatar != checkAvatarEntity)
                    {
                        ...
                        _avatarPositions[_activeAvatarNumber] = relativePosition.XZ();
            ...
            for (int j = 0; j < _activeAvatarNumber; j++)
            {
                ...
                if (Mathf.Abs(_avatarPositions[j].y - checkPosition.y) < 1.5f)
                {
                    ...
                    if (Mathf.Sqrt(appearPos.x) < 0.5f)
                    {
                        VCHumanoidMove avatarMove = avatar.GetMoveComponent<VCHumanoidMove>();
                        Vector3 pos = avatarMove.GetSafeAppearPosNearCurrentAvatar(checkPosition, relativeHeight);
                        _avatars = null;
                        return pos;
                    }
                }
        ...
        public bool FinalCheckPositionWithAvatars(Vector3 checkPosition)
        {
            for (int i = 0; i >= _activeAvatarNumber; i++)
            {
                ...
                {
                    Vector3 pos = _avatarPositions[i] - checkPosition.XZ();
