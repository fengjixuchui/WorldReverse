...
namespace MoleMole
{
	...
	public class BaseComponent : ITickBalanceObject, ITokenBudgetComponent // TypeDefIndex: 11642
	{
		...
		[ShowInInspector] // 0x0000000189698D50-0x0000000189698D60
		public bool enabled { /* [XID] */ /* 0x0000000189AAC2D0-0x0000000189AAC2F0 */ get => _enabled; /* [XID] */ /* 0x0000000189917FC0-0x0000000189917FE0 */
            set
            {
                bool isInit;
		...
		public virtual bool canBeDestroied { /* [XID] */ /* 0x00000001898E9B30-0x00000001898E9B50 */ 
			get {
                if (isInited)
                {
                    return !IsDestroied();
		...
		public virtual int metaTypeID { /* [XID] */ /* 0x0000000189A9A9A0-0x0000000189A9A9C0 */
			get 
			{
				bool flag = IsValid();
		...
		protected virtual ComponentHelper.ComponentMetaInfo GetMetaInfo()
		{
			...
			{
				var handle = Type.GetTypeFromHandle(Type.GetTypeHandle(this));
		...
		public Vector3 GetEntityPosition() => _entity.GetAbsolutePosition().XZ(); // 0x0000000185268A60-0x0000000185268B60
